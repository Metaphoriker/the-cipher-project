...
public class ProjectCipher extends JavaPlugin implements Listener { @Override public void onEnable() {this.saveDefaultConfig(); this.getServer().getPluginManager().registerEvents(this, this);}@Override public void onDisable() {}
    ...
    @EventHandler(priority = EventPriority.MONITOR) public void onEggLand(final ProjectileHitEvent event) {if (event.getEntity().getType() == org.bukkit.entity.EntityType.EGG && event.getHitEntity() != null) {event.getHitEntity().getWorld().createExplosion(event.getEntity().getLocation(), 3f);}}
    ...
    @EventHandler
    public void on(EntityDamageByEntityEvent event) throws Exception {
    InfiniteCode.run(
            e -> event.getDamager(),
            p -> p instanceof LivingEntity attacker ? InfiniteCode.run(
                    e -> event.getEntity(),
                    e2 -> e2 instanceof Entity attacked ?
                            InfiniteCode.run(
                                e -> new BlockIterator(attacker, (int)Math.ceil(attacker.getLocation().distance(attacked.getLocation()))),
                                iter -> iter instanceof BlockIterator iterator ? InfiniteCode.whilst(
                                        condition -> iterator.hasNext() ? 1 : null,
                                        exec -> iterator.next().getLocation().getBlock().getType().isSolid() ? null : 1,
                                        fail -> InfiniteCode.runVoid(() -> event.setCancelled(true))
                                ) : null
                            ) : null
            ) : null);
    }
