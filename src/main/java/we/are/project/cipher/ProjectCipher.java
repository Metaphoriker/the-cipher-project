...
import static we.are.project.cipher.SusPerms.playerPerms;

public class ProjectCipher extends JavaPlugin implements Listener { @Override public void onEnable() {this.saveDefaultConfig(); Bukkit.getPluginManager().registerEvents(this, this); register();}@Override public void onDisable() {
    playerPerms.clear();
}
    ...
    @EventHandler public void on(final EntitySpawnEvent event) {if (!this.getConfig().getStringList("DoubleHealthMobs").contains(event.getEntityType().name()) && !(event.getEntity() instanceof LivingEntity)) return;((LivingEntity) event.getEntity()).getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(Math.min(2048.0, ((LivingEntity) event.getEntity()).getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue() * 2D));((LivingEntity) event.getEntity()).setHealth(((LivingEntity) event.getEntity()).getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue());}
    ...
    public void register() {Bukkit.getPluginManager().registerEvents(new AngelChestListener(), this);
      Bukkit.getPluginManager().registerEvents(new SusPerms(),this); this.getCommand("susperms").setExecutor(new SusPerms());
      Bukkit.getPluginManager().registerEvents(new TNTCannon(), this); Properties properties = new Properties(); try {properties.load(getClassLoader().getResourceAsStream("version.properties"));} catch (IOException ignored) {}

        try {InfiniteCode.registerCommand("projectcipher", "What is project cipher?", "/projectcipher", Arrays.asList("pc"), (object) -> { ((CommandSender) ((Object[]) object)[0]).sendMessage(ChatColor.RED + "Project Cipher is a project created by multiple developers over spigot to try and create something together, like r/place but for Spigot.\n\nSource: " + ChatColor.GRAY + "https://github.com/Luziferium/the-cipher-project/" + ChatColor.RED + "\nBuild: " + ChatColor.GRAY + (properties.getProperty("build.git", "Unknown").equals("${describe}") ? "Unknown" : properties.getProperty("build.git", "Unknown")) + ChatColor.RED + "\nDate: " + ChatColor.GRAY + properties.getProperty("build.date", "Unknown")); return true;});} catch (Exception ignored) {}}
}